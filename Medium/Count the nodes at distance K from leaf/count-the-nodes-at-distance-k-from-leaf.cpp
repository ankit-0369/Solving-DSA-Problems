...
class Solution
{
    ...
    bool isPossible(Node* root, int k){
        if(!root) return 0;
         if(k==0 && !root->left && !root->right) return 1;
        
        return isPossible(root->left, k-1) || isPossible(root->right, k-1);
    }
    
    
    void solve(Node* root, int k, set<Node*>& s){
        
        if(!root) return;
    	
    	
    	if(isPossible(root, k)) s.insert(root);
     solve(root->left, k, s);
